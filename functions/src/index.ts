...
export const onTaskEventCreated = onDocumentCreated("taskEvents/{id}", async (event: any) => {
  const snap = event.data;
  if (!snap) return;

  const data = snap.data() as Partial<TaskEvent>;
  const docRef = snap.ref;

  // 1) 入力の軽い検証
  const required = ["storeId","reservationId","table","course","taskLabel","timeKey","date","dedupeKey","deviceId"] as const;
  ...
  try {
    await db.runTransaction(async (trx: FirebaseFirestore.Transaction) => {
      const lock = await trx.get(lockRef);
      if (lock.exists) throw new Error("already-processed");
  ...
  const token = tokenDoc.get("token") as string | undefined;
  if (!token) {
